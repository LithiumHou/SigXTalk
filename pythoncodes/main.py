...
def XGBmodel_single(cur_row,exp):
    ...
    df_imp = pd.DataFrame(list(importance.items()), columns=['Signal', 'PRS'])
    df_imp['SSC'] = cur_SSC
    df_imp['TG'] = cur_TG
    df_imp = df_imp[['Signal','SSC','TG','PRS']]
...
# Parse the args for hyperlink prediction
    ...
    print("Loading expression data and databases ...")
